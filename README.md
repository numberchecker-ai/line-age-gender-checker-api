# Line Profile Checker API — Bulk Verification | NumberChecker

The **Line Profile Checker API** accepts a text file of phone numbers, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in seven languages.

- **Product page:** https://numberchecker.ai/products/line-age-gender-checker
- **API documentation:** https://docs.numberchecker.ai/line-profile-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `line_senior` only. Other product codes are outside its scope.

## Product facts

| API fact | Value |
|---|---|
| Product code | `line_senior` |
...
The current catalog notes: No profile columns appeared in the selected live sample; current profile-field availability remains unverified.

## Workflow

1. Submit the input file to `POST /v1/tasks` with `task_type=line_senior`.
...
Only `line_senior`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/line-age-gender-checker
- **Documentation:** https://docs.numberchecker.ai/line-profile-checker
